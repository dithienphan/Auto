...
> `create-db-auto.sh` und `create-db-auto.sql` aus dem Verzeichnis
...
> `C:\Zimmermann\volumes\postgres\tablespace\auto` muss außerdem Vollzugriff
...
> Datenbank-User `auto` und dessen Datenbank `auto` anlegen, d.h. der neue
> Datenbank-User `auto` wird zum Owner der Datenbank `auto`. Dazu muss man
...
> ❗ Vor dem 1. Start von MySQL muss man die Skripte `create-db-auto.sh` und
> `create-db-auto.sql` aus dem Projektverzeichnis
...
> `auto` und dessen Datenbank `auto` anlegen, d.h. der neue Datenbank-User
> `auto` wird zum Owner der Datenbank `auto`. Dazu muss man sich mit dem
...
- `helm install auto . -f values.yaml -f dev.yaml` in `extras\helm`
...
welches durch _Mounting_ dem Windows-Verzeichnis `C:\Zimmermann\volumes\auto`
...
- `helm uninstall auto` in `extras\helm`
