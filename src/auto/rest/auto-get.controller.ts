...
export class AutoQuery implements Suchkriterien {
    ...
    @ApiProperty({ required: false })
    declare readonly hersteller: Hersteller;
    ...
    @ApiProperty({ required: false })
    declare readonly suv: boolean;

    @ApiProperty({ required: false })
    declare readonly kombi: boolean;
}
...
export class AutoGetController {
     ...
     * `304` (`Not Modified`) zurückgeliefert.
     *
     * Falls `If-None-Match` nicht
     ...
     *
     * Falls es mindestens ein solches Auto gibt, wird der Statuscode `200` (`OK`) gesetzt.
     * Im Rumpf des Response ist das JSON-Array mit den gefundenen Autos, die jeweils
